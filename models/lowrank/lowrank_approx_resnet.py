...
def approx_lowrank_weights(orig_model, orig_weights, conf,
	...
	if layer_name == "conv1" :
		...
		W = np.asarray([p.data for p in orig_net.params[layer_name]])
        	v_weights = np.asarray(\
            	  [p.data for p in lowrank_net.params[layer_name + '_v']])
        	h_weights = np.asarray(\
            	  [p.data for p in lowrank_net.params[layer_name + '_h']])
		v_bias = np.ndarray([])
		h_bias = np.ndarray([])
        # Set biases
	if layer_name == "conv1" :
	 	...
	 	v_bias[...] = 0
        	h_bias[...] = 0
        # Get the shapes
        num_groups = v_weights.shape[1] // h_weights.shape[2]
	if layer_name == "conv1":
        	N, C, D, D = W.shape
	else:		
        	SMTH, N, C, D, D = W.shape
        N = N // num_groups
	print(W.shape)
        K = h_weights.shape[2]
        # SVD approximation
        for g in xrange(num_groups):
		 ...
		 W_ = W[N*g:N*(g+1)].transpose(0,2, 3, 4, 1).reshape((C*D, D*N))
