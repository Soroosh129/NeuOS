...
inline int schedule(float priority, float &slack,  float Uncertainty, int  current_configuration)
{
	if(slack < 0)
	...
	else
	{
		if(Uncertainty>3)
		{
			current_configuration = priority*current_configuration - 10.0/Uncertainty;
		}
		else
		{
			current_configuration = priority*current_configuration +  100.0/Uncertainty;
		}
	}
...
inline int schedule(float priority, float &slack, float expected_deadline, float measured_exec, float Uncertainty, int  current_configuration)
{
	slack += measured_exec - expected_deadline;
	
	return schedule(priority, slack, Uncertainty, current_configuration);
}

inline int schedule(float priority, float &slack, float expected_deadline, float previous_expected_deadline, float measured_exec, float Uncertainty, int prev_current_configuration,int current_configuration, std::map<int,hist_ent> &prev_history, std::map<string,int> &rev_prev_history, std::map<int,hist_ent> &history, std::map<string,int> &rev_history)
{
...
/* to-do: use boost multi index */	
	...
	{ 
		...
		current_configuration = schedule(priority, slack, Uncertainty, current_configuration);
	}
	return current_configuration;
}
