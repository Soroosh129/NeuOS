...
int main(){
	ifstream infile("vggnet-med.config");
	string line;
	dvfs dv(2);
	while(getline(infile, line)){
		...
		string s;
		ss >> s;
		string filename = "config-"+s+".log";
		string cmdStr ="/home/nvidia/caffe-build/classify.sh vggnet > /home/nvidia/caffe-build/dvfs/vggnet-cudnn/"+filename;
		system(cmdStr.c_str());
