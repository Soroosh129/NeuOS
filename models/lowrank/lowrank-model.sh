...
executable="lowrank_approx_"$model".py"
echo $uncert
case "$model" in
	...
	"vggnet") weight="/home/nvidia/caffe-build/models/vggnet/VGG_ILSVRC_19_layers.caffemodel" ;
		proto="/home/nvidia/caffe-build/models/vggnet/VGG_ILSVRC_19_layers_deploy_V2.prototxt" ;;
...
python $executable   --model $proto   --config $model"-config.json"   --save_model $model"_lowrank_deploy.prototxt" --weights $weight --save_weights $model"_lowrank.caffemodel"
