...
namespace caffe {
	...
	template <typename Dtype>
		bool Net<Dtype>::myCompare(config c1, config c2){
			...
			if(c1.mem_freq == c2.mem_freq && 
					c1.CPU_freq == c2.CPU_freq &&
					c1.GPU_freq == c2.GPU_freq &&
					getCores(c1.CPU_mask, 8) < getCores(c2.CPU_mask, 8))
				return true;
			//	if(c1.mem_freq == c2.mem_freq && 
	...
	template <typename Dtype>
		void Net<Dtype>::setup_DVFS(float period, const string config_path, const string Uncertainty_path)
		{
			...
			while(getline(fconf,tmp))
			{
				...
				conf.CPU_freq = std::stoi(confstr[1]);
				conf.CPU_mask[0] = std::stoi(confstr[2]);
				conf.CPU_mask[1] = std::stoi(confstr[3]);
				conf.CPU_mask[2] = std::stoi(confstr[4]);
				conf.CPU_mask[3] = std::stoi(confstr[5]);
				//conf.large_freq = std::stoi(confstr[6]);
				conf.CPU_mask[4] = std::stoi(confstr[6]);
				conf.CPU_mask[5] = std::stoi(confstr[7]);
				conf.CPU_mask[6] = std::stoi(confstr[8]);
				conf.CPU_mask[7] = std::stoi(confstr[9]);
				conf.GPU_freq = std::stoi(confstr[10]);
				conf.mem_freq = std::stoi(confstr[11]);
				raw.push_back(conf);
				//DVFS_print(conf);
				//std::cout << conf.large_freq << "\t" << conf.small_freq << "\t" << conf.GPU_freq << std::endl;
			}
	...
	template <typename Dtype>
		void Net<Dtype>::warmup()
		{
			...
			setCoresOnline(cur.CPU_mask);
			//setBigCoresOnline(cur.large_mask);	
			setClusterFreq(cur.CPU_freq);
			...
			for (int i = start; i <= end; ++i) {
				...
				if(!isWarmup)
				{
					...
					if(i > 0)
					{
						...
						bool flag = true;
						for(int c=0;c<8;c++){
							if(last.CPU_mask[c]!=cur.CPU_mask[c]){
								flag=false;
								break;
							}
						}
